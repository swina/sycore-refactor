...
AI is a tool. That's the whole position, stated plainly.

In this project, AI serves two roles — one for building the platform, and one for the person using it.

For building: AI writes and refactors code, catches bugs, runs tests, and helps ship updates faster than I could alone. It's a workflow accelerant for the *software* — the same category as a compiler or a linter, just a lot more capable. It has no say in what SY.CORE is for or what it should feel like to use. Every principle in this manifesto — extension not replacement, local-first, tools that disappear, the free presets — came from years of playing an S-1 and getting frustrated at 2am, not from a model. AI didn't design this platform. I did.

For using: AI shows up in ways that help the musician learn, explore, and move faster without taking creative control. A chord suggestion modal that proposes voicings you hadn't thought of. A generative patch engine that understands what makes a *pad* a pad in a specific synth's parameter space. An arp pattern or step-sequencer fill that gives you a starting point to tweak. Each of these is a tool you direct, not a composer you hand the reins to. You still decide whether the chord works, whether the patch fits the track, whether the pattern stays or gets trashed. The AI doesn't make music — it proposes materials, explains concepts, and gets out of the way the moment you start playing.

That line is important: AI should help you learn music theory, understand how your synth works, discover voicings you wouldn't have found on your own, or automate the repetitive parts of a workflow so you can focus on the decisions that actually matter. It should never write a song for you, never generate a finished performance you can call your own without having made a single musical choice. A tool that helps you learn an instrument faster is fundamentally different from a tool that plays the instrument for you.

The generative engine described earlier in this manifesto — the one that proposes patches for the S-1 — lives right on that line. It doesn't write songs or make musical decisions for you. It proposes *parameter values* inside a synth you're already playing, the same way a well-designed randomize button or a preset does, just with a better ear for what makes a pad a pad in this specific instrument's parameter space. You still have to play it, shape it, decide if it's a bass or garbage, and turn it into actual music.

If SY.CORE ever ships something described as AI-assisted, the test is the same one this whole manifesto keeps coming back to: does it help a musician play or create *their own* music faster, or does it try to do the musician's job for them? The first is a tool. The second is a shortcut that steals the craft. This project is built for the first kind only.
