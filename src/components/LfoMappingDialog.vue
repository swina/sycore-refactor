...
<template>
  <ModalShell size="sm" z-class="z-[500]" @close="close">

    <template #header>
      <div class="flex items-center gap-3">
        <div class="p-2 bg-synth-cyan/10 rounded-lg">
          <Activity class="w-5 h-5 text-synth-cyan" />
        </div>
        <h2 class="text-sm font-black uppercase tracking-widest text-synth-cyan">LFO {{ props.lfoId }} ENGINE</h2>
      </div>
    </template>
        ...
        <div class="space-y-2 relative" @contextmenu.prevent="openMenu($event, { name: 'lfo' + lfoId + '_waveform', label: 'LFO ' + lfoId + ' Waveform' })">
          ...
          <div class="relative">
            <select
              ...
              class="w-full bg-neutral-900 border border-neutral-800 rounded p-2.5 text-[10px] font-mono uppercase tracking-widest text-white outline-none focus:border-synth-cyan transition-colors appearance-none cursor-pointer"
            >
          ...
          <div class="flex bg-neutral-900 rounded p-1 border border-neutral-800">
            <button
              ...
              :class="['flex-1 py-1.5 text-[10px] font-mono font-bold uppercase tracking-widest rounded transition-all',
                config.mode === m ? 'bg-neutral-800 text-synth-cyan' : 'text-neutral-500 hover:text-neutral-300']"
            >
              {{ m }}
            </button>
          </div>
      ...
      <div class="space-y-2 relative" @contextmenu.prevent="openMenu($event, { name: 'lfo' + lfoId + '_target', label: 'LFO ' + lfoId + ' Target' })">
        ...
        <div class="relative group">
          <select
            ...
            class="w-full bg-neutral-900 border border-neutral-800 rounded p-3 text-xs font-mono uppercase tracking-widest text-white outline-none focus:border-synth-cyan transition-colors appearance-none cursor-pointer"
          >
        ...
        <div class="space-y-2 relative" @contextmenu.prevent="openMenu($event, { name: 'lfo' + lfoId + '_rate', label: 'LFO ' + lfoId + ' Rate' })">
          ...
          <template v-if="config.mode === 'free'">
            <input
              ...
              class="w-full accent-synth-cyan h-1 bg-neutral-800 rounded-lg appearance-none cursor-pointer"
            />
          </template>
          <template v-else>
            <input
              type="range" min="0" :max="lfoStore.SYNC_DIVISIONS.length - 1" step="1"
    ...
    <template #footer>
      <div class="p-5 border-t border-neutral-800/50">
